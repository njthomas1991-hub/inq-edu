...
const KindlewickTeacherPanel = ({ user }) => {
  const [filters, setFilters] = useState({ classId: '', studentId: '', teacherId: '' });
  ...
  const basePath = useMemo(() => {
    if (!user) return null;
    if (user.role === 'school_admin') return '/kindlewick/school-admin';
    if (user.role === 'teacher') return '/kindlewick/teacher';
    return null;
  }, [user]);
  ...
  const buildQuery = () => {
    ...
    if (filters.teacherId && user?.role === 'school_admin') params.set('teacher_id', filters.teacherId);
  ...
  useEffect(() => { if (basePath && expanded) loadTracking(); }, [basePath, expanded]);
  ...
  return (
    <section className="kw-react-card kw-teacher-panel">
      <header className="kw-teacher-header">
        ...
        <button type="button" onClick={() => setExpanded((prev) => !prev)} aria-expanded={expanded} aria-controls="kw-teacher-panel-content">
      ...
      {expanded && (
        <div id="kw-teacher-panel-content">
          <div className="kw-teacher-filters">
            <label>
              Class ID
              <input type="text" name="classId" value={filters.classId} onChange={handleFilterChange} placeholder="Optional" />
            </label>
            <label>
              Student ID
              <input type="text" name="studentId" value={filters.studentId} onChange={handleFilterChange} placeholder="Optional" />
            </label>
            {user?.role === 'school_admin' && (
              <label>
                Teacher ID
                <input type="text" name="teacherId" value={filters.teacherId} onChange={handleFilterChange} placeholder="Optional" />
              </label>
            )}
          </div>

          {status && <div className={`kw-react-alert ${status.type}`}>{status.message}</div>}
