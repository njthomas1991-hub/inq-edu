...
def _resolve_create_student_account_view(request, *args, **kwargs):
    ...
    view = getattr(api_views, "create_student_account_view", None) or getattr(
        api_views, "create_student_account", None
    )
    if view is None:
        raise AttributeError(
            "No create student account view found on core.views.api_views"
        )
...
def _resolve_avatar_view(request, *args, **kwargs):
    ...
    view = getattr(avatar_views, "get_user_avatar", None) or getattr(
        avatar_views, "save_user_avatar", None
    )
    if view is None:
...
urlpatterns = [
    ...
    path("api/avatar/", _resolve_avatar_view, name="get_avatar"),
    path("api/avatar/save/", _resolve_save_avatar_view, name="save_avatar"),
    path(
        "api/avatar/randomize/", _resolve_randomize_avatar_view, name="randomize_avatar"
    ),
    ...
    path(
        "api/kindlewick/sessions/", kindlewick_sessions, name="api_kindlewick_sessions"
    ),
    path(
