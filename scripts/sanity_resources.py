...
u, created = User.objects.get_or_create(
    ...
    defaults={'email': 'test_teacher@example.com', 'first_name': 'Test', 'role': 'teacher'}
)
if created:
...
for i in range(1, 15):
    ...
    if not TeachingResource.objects.filter(title=title, teacher=u).exists():
        ...
        res = TeachingResource(teacher=u, title=title, description='Auto-generated for smoke test')
