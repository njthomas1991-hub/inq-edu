...
try:
    cursor.execute(
        "ALTER TABLE core_classstudent RENAME COLUMN clazz_id TO class_obj_id"
    )
...
try:
    ...
    for constraint in constraints:
        try:
            cursor.execute(
                f"ALTER TABLE core_classstudent DROP CONSTRAINT {constraint[0]}"
            )
    ...
    cursor.execute(
        "ALTER TABLE core_classstudent ADD CONSTRAINT core_classstudent_student_fk FOREIGN KEY (student_id) REFERENCES core_user(id)"
    )
