...
resp = c.post(
    "/login/",
    {"username": email, "password": password, "remember": "on"},
    **server_kwargs,
)
...
if user:
    resp2 = c.post(
        "/login/",
        {"username": user.username, "password": password, "remember": "on"},
        **server_kwargs,
    )
    print("Login POST status with username:", resp2.status_code)
    print(
        "Session _auth_user_id after username login attempt:",
        c.session.get("_auth_user_id"),
    )
